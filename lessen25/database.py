...
def update_movie(movie_id: int, movie: Movecreate) -> bool: 
    connection = create_connection()
    cursor = connection.cursor()
    cursor.execute("UPDATE movies SET tittle = ?, description = ? WHERE id = ?", (movie.tittle, movie.description, movie_id))
    connection.commit()
